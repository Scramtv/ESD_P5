...
class MotorCtrl:
    ...
    def __build_motor_packet(self, motor: bool, degrees: int) -> bytes:
        ...
        motor_bit = int(format(motor, 'b'))
        # Forces to only keep 12 bits (0xFFF is just to tell python this)
        degrees_bits = degrees & 0xFFF

        # Shifts motor_bit to MSB, and shifts degrees 3 to have 3 zeros in the end
        packed = (motor_bit << 15) | (degrees_bits << 3)
